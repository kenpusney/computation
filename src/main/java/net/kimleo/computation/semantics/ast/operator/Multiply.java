...
public class Multiply implements ReducibleExpression<Integer> {


    private final Valuable<Integer> left;
    private final Valuable<Integer> right;

    public Multiply(Valuable<Integer> left, Valuable<Integer> right) {
        this.left = left;
        this.right = right;
    }
    ...
    public Valuable<Integer> reduce(Environment environment) {
        if (left instanceof ReducibleExpression) {
            return new Multiply(((ReducibleExpression<Integer>) left).reduce(environment), right);
        } else if (right instanceof ReducibleExpression) {
            return new Multiply(left, ((ReducibleExpression<Integer>) right).reduce(environment));
        } else {
